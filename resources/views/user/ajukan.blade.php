...
@section('content')
    ...
    <div class="flex items-center justify-between bg-white p-4 rounded-xl shadow-sm border border-slate-200 mb-6">
        ...
        <div class="text-center">
            <h3 id="current-label" class="text-lg font-extrabold text-[#002045] transition-all">Memuat...</h3>
        </div>
        <button onclick="navigateDate(1)" class="p-2 bg-slate-50 hover:bg-slate-100 rounded-lg text-[#002045] transition-colors border border-slate-200 active:scale-95">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-4 mb-6 flex flex-col xl:flex-row xl:items-center justify-between gap-4 transition-all">
        ...
        <div id="filter-inputs" class="flex flex-wrap items-center gap-3 w-full xl:w-auto">
            <select id="filterGedung" onchange="applyFilter()" class="flex-1 min-w-[160px] bg-slate-50 border border-slate-200 rounded-lg py-2.5 px-4 text-sm font-medium focus:ring-2 focus:ring-[#002045]/20 outline-none">
                <option value="">Semua Gedung</option>
                <option value="A">Gedung A (Rektorat)</option>
                <option value="B">Gedung B (Fak. Teknik)</option>
                <option value="C">Gedung C (Fak. Ekonomi)</option>
            </select>
            <select id="filterRuangan" onchange="applyFilter()" class="flex-1 min-w-[160px] bg-slate-50 border border-slate-200 rounded-lg py-2.5 px-4 text-sm font-medium focus:ring-2 focus:ring-[#002045]/20 outline-none">
                <option value="">Semua Ruangan</option>
                @foreach($rooms as $r)
                    <option value="{{ $r['code'] }}" data-gedung="{{ $r['gedung'] }}">{{ $r['name'] }}</option>
                @endforeach
            </select>
            <input id="filterTanggal" class="flex-1 min-w-[160px] bg-slate-50 border border-slate-200 rounded-lg py-2.5 px-4 text-sm font-medium focus:ring-2 focus:ring-[#002045]/20 outline-none" type="date">
        </div>
    </div>
    ...
    <div class="relative">
        <div id="view-bulanan" class="transition-all duration-300">
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden mb-6">
                <div class="grid grid-cols-7 border-b border-slate-200 bg-slate-50">
                    ...
                    <div class="p-3 text-center border-r border-slate-200 last:border-0"><span class="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{{ $day }}</span></div>
                    @endforeach
                </div>
        ...
        <div id="view-mingguan" class="hidden transition-all duration-300">
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden mb-6">
                <div class="overflow-x-hidden"><div id="view-mingguan-inner" class="w-full relative bg-white"></div></div>
            </div>
        </div>
    ...
    {{-- Pop up Detail & Aksi Pengajuan (USER VERSION) --}}
            ...
            <div class="p-6">
                ...
                <div id="popupActionAvailable" class="hidden">
                    ...
                    <div class="flex gap-3">
                        <button onclick="closePopupBtn()" class="flex-1 py-2.5 border border-slate-200 rounded-lg text-sm font-bold text-slate-600 hover:bg-slate-50">Batal</button>
                        <a id="popupSelectBtn" href="#" class="flex-1 py-2.5 bg-primary-gradient text-white rounded-lg text-sm font-bold text-center hover:opacity-95 flex items-center justify-center gap-2">
                            Pilih Slot Ini <span class="material-symbols-outlined text-sm">arrow_forward</span>
                        </a>
                    </div>
                </div>
                ...
                <div id="popupActionBooked" class="hidden">
                    ...
                    <button onclick="closePopupBtn()" class="w-full py-2.5 border border-slate-200 rounded-lg text-sm font-bold text-slate-600 hover:bg-slate-50">Tutup</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    function renderDailyCalendar() {
        ...
        roomsData.forEach(room => {
            html += `<div class="room-col flex-1 min-w-[160px] px-3 py-3 border-r border-slate-200 last:border-r-0 bg-slate-50 text-center" data-code="${room.code}" data-gedung="${room.gedung}">
        ...
        hours.forEach(h => {
            ...
            roomsData.forEach(room => {
                ...
                if(slotEvent) {
                    ...
                    html += `<button onclick="selectSlot(this, '${room.code}', ${h}, '${room.name}', '${room.building}', ${room.capacity}, '${room.img}', '${room.desc}')"
                                     ...
                                     data-room="${room.code}" data-hour="${h}" data-status="available">
