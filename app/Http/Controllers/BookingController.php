...
class BookingController extends Controller
{
    public function ajukan()
    {
        ...
        $rooms = $dbRooms->map(function ($room) {
            return [
                ...
                'desc'    => $room->facilities ?? 'Fasilitas standar tersedia.',
            ];
        })->toArray();

        $dbBookings = \App\Models\Booking::with('room')
            ->whereIn('status', ['menunggu', 'disetujui'])
            ->get();

        $bookingsList = $dbBookings->map(function ($booking) {
            $room = $booking->room;
            if (!$room) {
                return null;
            }
            ...
            return [
                ...
                'booker'    => $booking->user->name ?? 'Pemohon',
            ];
    ...
    public function ajukanDetail(Request $request)
    {
        ...
        $selectedHour = (int) $request->query('hour');
    ...
    public function ajukanKonfirmasi(Request $request)
    {
        ...
        if ($isBooked) {
            return back()->withErrors([
                'room_id' => 'Maaf, ruangan ini sudah terpesan pada jam tersebut. Silakan pilih waktu lain.',
            ])->withInput();
        }
